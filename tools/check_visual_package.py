...
def main():
    ...
    for name,expected in manifest.get('tracker_assets',{}).items():
    ...
    tracker_enabled=manifest.get('camera_tracker','none')!='none'
    if tracker_enabled:assert len({id(a.visual_tracker.tracker) for a in agents})==3
    ...
    for a in agents:
        ...
        if tracker_enabled:
            import cv2
            import numpy as np
            pixels=cv2.imdecode(np.frombuffer(photo,np.uint8),cv2.IMREAD_COLOR)
            ok,encoded=cv2.imencode('.jpg',pixels,[cv2.IMWRITE_JPEG_QUALITY,95]);assert ok
            current=encoded.tobytes();source_hash=hashlib.sha256(photo).hexdigest();current_hash=hashlib.sha256(current).hexdigest()
            assert a.boxes and current_hash!=source_hash
            with (patch('builtins.open',side_effect=AssertionError('tracker callback file IO')),
                  patch('io.open',side_effect=AssertionError('tracker callback file IO')),
                  patch('os.open',side_effect=AssertionError('tracker callback file IO'))):
                assert a.visual_tracker.remember(photo,a.boxes[0],source_hash,1.,('private',1))
                region=a.visual_tracker.propose(current,current_hash,1.5,np.eye(3),source_hash,current_hash,('private',1))
                assert region is not None
                classified=a.detector.classify_regions(current,[region])
    ...
    out.write_text(json.dumps(dict(status='passed',agent_sha256=digest(path),vision_sha256=manifest['vision_sha256'],
        ...
        private_trackers=tracker_enabled,tracker_update_and_region_classification_without_python_file_io=tracker_enabled,
