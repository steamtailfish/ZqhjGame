...
p=argparse.ArgumentParser();p.add_argument('--output',type=Path,required=True);p.add_argument('--max-wall',type=float,default=1200);a=p.parse_args()
...
rows=[];last=-float('inf');started=time.monotonic();last_message=started;reason='timeout'
try:
