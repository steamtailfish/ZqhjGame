...
原始记录位于 `artifacts/vision/runs/score600-v22-seed101/official/coop_decoy_1789108334.evaluation.json`。冻结包 `baseline_evaluation.json` 是原样副本。二者均通过 Git LFS 随仓库提供；对应哈希见[资产清单](ARTIFACT_HANDOFF.md)。当前结果见 [FIRST_SCORE.md](../FIRST_SCORE.md)。
...
每个正式实验记录：Git 提交号、导出包/权重 SHA256、完整命令、环境版本、seed、实际仿真结束时间、官方 JSON 路径、总分、报告数、RMSE、清除数和惩罚。当前大体积资产使用 Git LFS，完整路径与哈希见 V22_ASSETS.json。尚未配置自动训练或 CI 回归。
